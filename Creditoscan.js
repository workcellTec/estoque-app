// ================================================================
// creditoscan.js — CréditoScan v13 (Anti-Fraude com Chain-of-Thought e JS Lock)
// ================================================================
(function () {
    'use strict';
    ...
    async function tesseractOCR(docs) {
        ...
        for (var i = 0; i < docs.length; i++) {
            try {
                setProg('Lendo página ' + (i + 1) + ' de ' + docs.length + '...', 10 + Math.round(55 * i / docs.length));
    ...
    // CORREÇÃO: Lê TUDO. Não apaga mais os nomes dos documentos!
    function filtrarLinhasRelevantes(textos) {
        return textos.map(function(texto, idx) {
...
'VALOR BASE: R$ '+vf+'\n\n'+
'=== REGRA DE TITULARIDADE (ANTI-FRAUDE) ===\n'+
'1) OBRIGATÓRIO: Identifique o nome do titular no Documento de Identidade (RG/CNH).\n'+
'2) Identifique o nome do titular no Comprovante de Renda (Holerite/Extrato).\n'+
'3) Se as pessoas forem DIFERENTES (ex: Identidade é Eduardo e Holerite é Sillone), isso é uma tentativa de FRAUDE.\n'+
'4) Você é obrigado a informar no JSON se "nomesBatem" é true ou false.\n\n'+
'=== PASSO 1 — EXTRAIR A RENDA REAL ===\n'+
'- EXTRATO: Some entradas reais. NÃO some transferências da própria pessoa.\n'+
'- HOLERITE: Renda Real = "VALOR LÍQUIDO". JAMAIS some descontos/empréstimos.\n\n'+
'=== REGRA DO HOLERITE E TEMPO DE EMPREGO ===\n'+
'- Descontos de "Empréstimo/Adiantamento": Rebaixa para FRACO (nota máx 59).\n'+
'- Menos de 3 meses de empresa: Rebaixa para FRACO.\n\n'+
'=== PASSO 2 — CLASSIFICAR O PERFIL ===\n'+
'- FORTE (80-100): Salário fixo/formal, regular, saldo positivo.\n'+
'- MÉDIO (60-79): Frequentes mas oscilam.\n'+
'- FRACO (40-59): Autônomo, gasta tudo, possui empréstimos consignados.\n'+
'- REPROVADO (0-39): Fraude de titularidade, ou saldo negativo crônico.\n\n'+
'=== PASSO 3 — BALANÇA DE RISCO (Entradas Mínimas) ===\n'+
'FORTE: 30% = R$ '+e30+' | MÉDIO: 35% = R$ '+e35+' | FRACO: 60% = R$ '+e60+'\n\n'+
'=== PASSO 4 — TESTE DE FOGO (Limite da Parcela) ===\n'+
'TETO DE PARCELA: FORTE (30% da renda) | MÉDIO (25%) | FRACO (10%).\n'+
'Se a parcela base ultrapassar o Teto, Mude a decisão para SUGESTAO_ENTRADA_ALTA e calcule a turbinada (Teto * 8,4 = Saldo Financiado Máximo).\n\n'+
'=== FORMATO DE SAÍDA OBRIGATÓRIO (JSON) ===\n'+
'{\n'+
'  "nomeDocumentoPessoal": "Extraia o nome que está no RG ou CNH (ou vazio se não tiver)",\n'+
'  "nomeComprovanteRenda": "Extraia o nome que está no Holerite ou Extrato",\n'+
'  "nomesBatem": true ou false,\n'+
'  "perfilCliente": "Vínculo e tipo de renda.",\n'+
'  "analiseFinanceira": "Como gasta e recebe.",\n'+
'  "rascunhoCalculos": "Cálculo da Parcela vs Teto.",\n'+
'  "rendaEstimada": 1500.00,\n'+
'  "nota": 70,\n'+
'  "nivel": "MÉDIO",\n'+
'  "decisao": "VENDER",\n'+
'  "entradaTurbinadaCalculada": 0.00,\n'+
'  "motivosReprovacao": "Preencha APENAS se reprovado ou se houver divergência de nomes."\n'+
'}\n'
        );
    }
    ...
    function extrair(texto) {
        ...
        try {
            ...
            // TRAVA DE SEGURANÇA JAVASCRIPT: Se os nomes não batem, mata a aprovação na hora!
            ...
            if (!nomesBatem) {
                ...
                nota = 0;
                obj.nivel = 'REPROVADO';
                obj.motivosReprovacao = 'Alerta de Fraude: Divergência de Titularidade.\nDocumento Pessoal: ' + (obj.nomeDocumentoPessoal||'Não achado') + '\nComprovante de Renda: ' + (obj.nomeComprovanteRenda||'Não achado');
            }
            ...
            // Pega o primeiro nome para o zap
            ...
            return {
                ...
                nomeCliente: 'Erro', nomeCompleto: 'Erro na IA',
                perfil: 'Erro de formatação.', analise: texto, rascunho: '', motivos: 'Falha do Llama. Analise novamente.'
            };
        }
    }
    ...
    function formatResult(texto, d, valorBase) {
        ...
        var html = '<h4 class="cs-rt">Perfil do Cliente: '+esc(d.nomeCompleto)+'</h4><p class="cs-rp">'+esc(d.perfil)+'</p><h4 class="cs-rt">Análise Financeira</h4><p class="cs-rp">'+esc(d.analise)+'</p>';
        if (d.reprovado && d.motivos) html += '<h4 class="cs-rt" style="color:#ef4444">Motivos da Reprovação</h4><p class="cs-rp" style="color:#fca5a5">'+esc(d.motivos).replace(/\n/g, '<br>')+'</p>';
    ...
    function usarPerfilHistorico(idx) {
        ...
        novoBtnOk.addEventListener('click', function() {
            ...
            if(el('cs_produto')) el('cs_produto').value = p;
            if(el('cs_valor')) el('cs_valor').value = val;
    ...
    function openOverlay(){var ov=el('cs_ov');if(ov){ov.classList.add('active');_docs=[];_busy=false;_last=null;if(el('cs_produto')) el('cs_produto').value='';if(el('cs_valor')) el('cs_valor').value='';el('cs_res').classList.add('cs-h');el('cs_prog').classList.add('cs-h');resetUIState();renderDocs();setBtnState();}}
    ...
    async function executar(){
        ...
        var prod=el('cs_produto').value.trim(), val=parseFloat(el('cs_valor').value.replace(',','.'))||0;
        if(!prod||!val){alert('Preencha produto e valor.');return;}

        if(_last && _last.dados && _last.docsHash === _docs.length) {
            var confirmar = confirm("Usar os documentos já analisados para simular este novo produto? (Economiza Tokens)");
            if (confirmar) {
                _last.produto = prod;
                _last.valor = val;
                _last.dados._produto = prod;
                _last.entradaMin = Math.ceil(val * _last.dados.entradaPct / 10) * 10;
                el('cs_res').innerHTML = formatResult(JSON.stringify(_last.dados.rawJson), _last.dados, val);
        ...
        try {
            ...
            var resp=await groqCallTexto(textoFiltrado,buildPrompt(prod,val),key);
            setProg('Finalizando...',90);
            
            var d=extrair(resp); 
            d._produto=prod; 
            var nomeExtraido = d.nomeCompleto; // Salva o nome inteiro no FB

            _last={nome:nomeExtraido, produto:prod, valor:val, dados:d, entradaMin: Math.ceil(val * d.entradaPct / 10)*10, docsHash: _docs.length};
