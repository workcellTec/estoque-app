// ================================================================
// creditoscan.js — CréditoScan v10 (Workcell CTW - UI Atualizada + Cache UI)
// ================================================================
(function () {
    'use strict';
    ...
    async function tesseractOCR(docs) {
        ...
        for (var i = 0; i < docs.length; i++) {
            try {
                ...
                var result = await worker.recognize('data:image/jpeg;base64,' + docs[i].base64);
    ...
    function filtrarLinhasRelevantes(textos) {
        var RELEVANTE = [/R\$[\s\d.,]+/i, /[\d]{1,3}[.,][\d]{3}[.,][\d]{2}/, /[\d]+[.,][\d]{2}/, /salário|salario|líquido|liquido|bruto|vencimento|desconto|inss|fgts|ir |irrf/i, /saldo|extrato|lançamento|lancamento|credito|débito|debito/i, /pix|ted|doc|boleto|parcela|empréstimo|emprestimo|financiamento/i, /entrada|saída|saida|transferência|transferencia|depósito|deposito|saque/i, /bet|esportiv|pixbet|blaze|vaidebet|betnacional|apostas?/i, /admissão|admissao|cargo|empresa|competência|competencia|holerite|contracheque/i, /janeiro|fevereiro|março|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro/i, /\d{2}\/\d{2}\/\d{4}|\d{2}\/\d{2}\/\d{2}/];
        var linhasFiltradas = [];
...
'- Se for EXTRATO BANCÁRIO: Some as entradas reais. Para não inflar a renda artificialmente, NÃO some transferências recebidas do próprio cliente.\n'+
'- Se for CONTRACHEQUE/HOLERITE: A Renda Real é EXATAMENTE o "VALOR LÍQUIDO". JAMAIS some "Adiantamento", "Empréstimo" ou descontos.\n\n'+
'=== REGRA DO HOLERITE (EMPRÉSTIMOS E ADIANTAMENTOS) ===\n'+
'Se o cliente tiver descontos de "Empréstimo", "Crédito Consignado" ou "Adiantamento" rebaixe o perfil para FRACO (nota máxima 59).\n\n'+
'=== ALERTA DE GIRO CEGO (SAÍDAS PARA A PRÓPRIA CONTA) ===\n'+
'Analise as SAÍDAS no extrato bancário.\n'+
'- TOLERÂNCIA: Até 5 transferências no mês para si mesmo, ou valores que somados não passem de 40% da renda, é NORMAL. NÃO tire pontos.\n'+
'- PUNIÇÃO: Se houver 6 OU MAIS transferências para si mesmo, OU se o valor ultrapassar 60% da Renda Estimada, rebaixe o perfil em APENAS UM NÍVEL (Ex: de FORTE para MÉDIO).\n\n'+
...
'=== REGRA DO TEMPO DE EMPREGO ===\n'+
'- SE TIVER MENOS DE 3 MESES DE ADMISSÃO: O perfil DEVE ser rebaixado para FRACO (nota máxima 59).\n'+
'- SE NÃO TIVER DATA ESCRITA: Aplique a Regra de Neutralidade.\n\n'+
'=== PASSO 3 — BALANÇA DE RISCO (Entradas Mínimas) ===\n'+
'FORTE  → Entrada mínima: 30% = R$ '+e30+'\n'+
'MÉDIO  → Entrada mínima: 35% = R$ '+e35+'\n'+
'FRACO  → Entrada mínima: 60% = R$ '+e60+'\n\n'+
...
'A sua resposta DEVE ser EXCLUSIVAMENTE o código JSON abaixo preenchido, sem formatação markdown, sem blocos de código.\n'+
'{\n'+
'  "perfilCliente": "Descreva o vínculo empregatício e tipo de renda.",\n'+
'  "analiseFinanceira": "Descreva como o cliente gasta e recebe.",\n'+
'  "rascunhoCalculos": "Rascunho matemático de Renda, Teto e Entrada Turbinada.",\n'+
...
'}\n'
    ...
    function extrair(texto) {
        ...
        try {
            ...
            return {
                ...
                perfil: 'Erro de formatação da IA.', analise: texto, rascunho: '', motivos: 'Falha do Llama. Analise novamente.'
            };
        }
    }
    ...
    async function salvarFB(nome, produto, texto, d) {
        try {
            ...
            var db=window._firebaseDB; if(!db)return;
            var now=Date.now();
            var resumoIA = d.aprov ? (d.rascunho || 'Aprovado conforme critérios.') : (d.motivos || 'Reprovado por risco financeiro.');
            await fb.push(fb.ref(db,SAVE_PATH),{
                ...
                vparcela:d.vparcela||null, taxa:d.taxa||null, total:d.total||null, resumo: resumoIA,
    ...
    function formatResult(texto, d, valorBase) {
        ...
        var html = '<h4 class="cs-rt">Perfil do Cliente</h4><p class="cs-rp">'+esc(d.perfil)+'</p><h4 class="cs-rt">Análise Financeira</h4><p class="cs-rp">'+esc(d.analise)+'</p>';
        if (d.reprovado && d.motivos) html += '<h4 class="cs-rt" style="color:#ef4444">Motivos da Reprovação</h4><p class="cs-rp" style="color:#fca5a5">'+esc(d.motivos)+'</p>';
        if (d.rascunho) html += '<h4 class="cs-rt">Rascunho de Cálculos</h4><p class="cs-rp" style="opacity:0.7; font-size: 0.8em;">'+esc(d.rascunho).replace(/\n/g, '<br>')+'</p>';
        ...
        if (!d.aprov) return '<div class="cs-ri">'+banner+nota_html+'<div class="cs-rb">'+html+'</div></div>';
        ...
        if (d.downgrade) return '<div class="cs-ri">'+banner+nota_html+wpp_block+'<div class="cs-rb">'+html+'</div></div>';
        ...
        var sim = (d.entradaAlta) 
            ? '<div class="cs-sim cs-sim-orange"><div class="cs-sim-ttl"><i class="bi bi-lightning-charge-fill"></i> Entrada Turbinada — Condição especial</div><div class="cs-sim-aviso cs-sim-aviso-orange"><i class="bi bi-lock-fill"></i> Entrada mínima travada em '+R$(enMin)+' ('+Math.round(d.entradaPct*100)+'% do valor)</div>'+sim_grid+'</div>'
            : '<div class="cs-sim"><div class="cs-sim-ttl"><i class="bi bi-sliders"></i> Simular condições</div><div class="cs-sim-aviso"><i class="bi bi-lock-fill"></i> Entrada mínima travada em '+Math.round(d.entradaPct*100)+'% · '+R$(enMin)+' · Perfil '+riscoLabel+'</div>'+sim_grid+'</div>';

        return '<div class="cs-ri">'+banner+nota_html+sim+wpp_block+'<div class="cs-rb">'+html+'</div></div>';
    }

        async function abrirHist() {
        // Injeta o CSS do novo botão dinamicamente para não precisar mexer na função de estilos
        ...
        try {
            var fb=await import('https://www.gstatic.com/firebasejs/11.9.1/firebase-database.js');
            var db=window._firebaseDB; if(!db){if(list)list.innerHTML='<div class="cs-hist-empty">Firebase não disponível</div>';return;}
            ...
            // Salva globalmente para podermos usar os dados ao clicar
            ...
            list.innerHTML=items.map(function(it, i){
                ...
                // Só mostra o botão de reaproveitar se o cliente foi aprovado em algum nível
                var btnReuso = ap ? '<button class="cs-btn-reuso" data-i="'+i+'"><i class="bi bi-person-check-fill"></i> Reaproveitar Perfil (0 Tokens)</button>' : '';

                return '<div class="cs-hitem" style="border-left: 4px solid '+tc+'"><div class="cs-hrow1"><span class="cs-hnome">'+esc(it.nomeCliente)+'</span><span style="font-size:.65rem;font-weight:800;color:'+tc+'">'+(ap?'✓ APROVADO':'✗ NEGADO')+'</span></div><div class="cs-hprod" style="margin-bottom:4px">📦 '+esc(it.produto)+'</div><div style="font-size:.68rem; color:rgba(255,255,255,0.5); font-style:italic; line-height:1.2; margin-bottom:8px">"'+esc(resumoLimpo)+'"</div><div class="cs-htags">'+tags+'</div><div class="cs-hfoot"><span>'+dt+'</span><span>Op: '+esc(it.operador)+'</span></div>'+btnReuso+'</div>';
            }).join('');

            // Ativa os cliques nos botões de reaproveitar
            list.querySelectorAll('.cs-btn-reuso').forEach(function(btn){
                btn.addEventListener('click', function(e){
                    usarPerfilHistorico(parseInt(e.currentTarget.dataset.i));
                });
            });
    ...
    // NOVA FUNÇÃO: PUXA O CLIENTE DO HISTÓRICO COM MODAL CUSTOMIZADO
    function usarPerfilHistorico(idx) {
        ...
        if (!modal) {
            ...
            modal.innerHTML = 
                '<div style="background:#0b1325;border:1.5px solid rgba(139,92,246,.3);border-radius:16px;width:100%;max-width:380px;padding:20px;display:flex;flex-direction:column;gap:12px;box-shadow:0 10px 40px rgba(0,0,0,0.6)">'+
                    '<div style="font-size:.95rem;font-weight:800;color:#fff;display:flex;align-items:center;gap:8px"><i class="bi bi-person-check-fill" style="color:#a78bfa"></i> Recompra: <span id="cs_rm_nome" style="color:#a78bfa"></span></div>'+
                    '<div style="font-size:.7rem;color:rgba(255,255,255,.5);margin-bottom:5px;line-height:1.4">Informe o novo produto e o valor exato para simular utilizando o perfil de crédito já aprovado em memória (0 Tokens).</div>'+
                    '<div class="cs-fld"><label class="cs-lbl">Novo Produto</label><input type="text" class="cs-in" id="cs_rm_prod"></div>'+
                    '<div class="cs-fld"><label class="cs-lbl">Valor (R$)</label><input type="number" class="cs-in" id="cs_rm_val" placeholder="Ex: 2500"></div>'+
                    '<div style="display:flex;gap:10px;margin-top:10px">'+
                        '<button id="cs_rm_cancel" style="flex:1;padding:12px;border-radius:10px;border:1px solid rgba(255,255,255,.1);background:transparent;color:#fff;font-family:\'Poppins\',sans-serif;font-size:.8rem;font-weight:700;cursor:pointer">Cancelar</button>'+
                        '<button id="cs_rm_ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;font-family:\'Poppins\',sans-serif;font-size:.8rem;font-weight:700;cursor:pointer">Simular</button>'+
                    '</div>'+
                '</div>';
        ...
        el('cs_rm_nome').textContent = it.nomeCliente;
        el('cs_rm_prod').value = it.produto;
        el('cs_rm_val').value = ''; 
        modal.style.display = 'flex';
        el('cs_rm_val').focus();

        var btnOk = el('cs_rm_ok');
        var novoBtnOk = btnOk.cloneNode(true);
        btnOk.parentNode.replaceChild(novoBtnOk, btnOk);

        novoBtnOk.addEventListener('click', function() {
            var p = el('cs_rm_prod').value.trim();
            var valStr = el('cs_rm_val').value;
            var val = parseFloat(valStr ? valStr.replace(',', '.') : '0');

            if (!p || !val) { alert('Preencha produto e valor para prosseguir.'); return; }

            var d = extrair(it.textoCompleto);
            d._produto = p;
            ...
            el('cs_nome').value = it.nomeCliente;
            el('cs_produto').value = p;
            el('cs_valor').value = val;

            el('cs_reuso_modal').style.display = 'none';
            el('cs_hist_ov').classList.remove('active');
            ...
            var res = el('cs_res');
            if(res) {
                res.innerHTML = formatResult(it.textoCompleto, d, val);
                res.classList.remove('cs-h');
            }
    ...
    function resetUIState() {
        if(el('cs_btn_analisar')) el('cs_btn_analisar').classList.remove('cs-h');
        if(el('cs_btn_duo')) el('cs_btn_duo').classList.add('cs-h');
    }
    ...
    function renderDocs(){
        ...
        g.innerHTML=_docs.map(function(d,i){ return '<div class="cs-dt">'+(d.dataUrl?'<img src="'+d.dataUrl+'" alt="">':'<div class="cs-dt-pdf"><i class="bi bi-file-earmark-pdf-fill"></i><span>PDF</span></div>')+'<button class="cs-dt-rm" data-i="'+i+'"><i class="bi bi-x"></i></button></div>'; }).join('');
        g.querySelectorAll('.cs-dt-rm').forEach(function(b){ b.addEventListener('click',function(e){ 
            _docs.splice(parseInt(e.currentTarget.dataset.i),1); 
            _last = null; resetUIState(); // Reseta botões se alterar fotos
            renderDocs();setBtnState(); 
        }); });
        updateCounter();
    }
    function updateCounter(){ var c=el('cs_counter'); if(!c)return; var n=_docs.length; c.textContent=n+'/'+MAX_DOCS+' arquivos'; c.style.color = n>=MAX_DOCS ? '#f87171' : n>0 ? '#4ade80' : 'rgba(255,255,255,.35)'; }
    function setBtnState(){ 
        var canRun = _docs.length>0 && !_busy;
        if(el('cs_btn_analisar')) el('cs_btn_analisar').disabled = !canRun; 
        if(el('cs_btn_nova')) el('cs_btn_nova').disabled = !canRun;
    }
    ...
    async function handleFiles(files){
        for(var i=0;i<files.length;i++){
            var f=files[i];
            if(_docs.length>=MAX_DOCS){ window.showCustomModal&&window.showCustomModal({message:'Máximo atingido.'}); break; }
            if(f.type.startsWith('image/')){ var b=await resizeToBase64(f);if(b)_docs.push({file:f,dataUrl:URL.createObjectURL(f),base64:b}); } 
            else if(f.type==='application/pdf'||f.name.toLowerCase().endsWith('.pdf')){ var imgs=await pdfToImages(f); if(imgs.length) imgs.forEach(function(im){if(_docs.length<MAX_DOCS)_docs.push({file:f,dataUrl:im.dataUrl,base64:im.base64});}); }
        }
        _last = null; resetUIState(); // Reseta botões se adicionar fotos
        renderDocs();setBtnState();
    }
    function upload(accept,capture){ var inp=document.createElement('input');inp.type='file';inp.accept=accept;inp.multiple=true;if(capture)inp.capture='environment';inp.onchange=function(e){handleFiles(e.target.files);};inp.click(); }
    function openOverlay(){var ov=el('cs_ov');if(ov){ov.classList.add('active');_docs=[];_busy=false;_last=null;el('cs_nome').value='';el('cs_produto').value='';el('cs_valor').value='';el('cs_res').classList.add('cs-h');el('cs_prog').classList.add('cs-h');resetUIState();renderDocs();setBtnState();}}
    function closeOverlay(){el('cs_ov')&&el('cs_ov').classList.remove('active');}

    async function executar(){
        if(_busy||!_docs.length)return;
        var key=await getGroqKey(); if(!key){alert('Chave não configurada.');return;}
        var nome=el('cs_nome').value.trim()||'Cliente', prod=el('cs_produto').value.trim(), val=parseFloat(el('cs_valor').value.replace(',','.'))||0;
        ...
        try {
            ...
            _last={nome:nome, produto:prod, valor:val, dados:d, entradaMin: Math.ceil(val * d.entradaPct / 10)*10, docsHash: _docs.length};
            ...
            // EXIBE BOTÕES DE ATUALIZAR/REFAZER
            el('cs_btn_analisar').classList.add('cs-h');
            el('cs_btn_duo').classList.remove('cs-h');

            wireRes();
        } catch(e){ alert('Erro: '+e.message); el('cs_prog').classList.add('cs-h'); } finally{_busy=false;setBtnState();}
    }

    // NOVA FUNÇÃO: RECALCULAR LOCAL SEM GASTAR TOKEN
    function recalcularLocal() {
        if(!_last || !_last.dados) return;
        var prod = el('cs_produto').value.trim();
        var valStr = el('cs_valor').value;
        var val = parseFloat(valStr ? valStr.replace(',', '.') : '0') || 0;
        if(!prod || !val) { alert('Preencha produto e valor nas caixas originais acima.'); return; }
        
        _last.produto = prod;
        _last.valor = val;
        _last.dados._produto = prod;
    ...
    function copiarWpp(){
        var t=el('cs_wpp_txt').innerText; if(!t)return;
        if(navigator.clipboard){ navigator.clipboard.writeText(t).then(function(){ el('cs_wpp_hint').innerHTML='<i class="bi bi-check-lg"></i> Copiado!'; setTimeout(function(){el('cs_wpp_hint').innerHTML='<i class="bi bi-clipboard-fill"></i> Toque no texto acima para copiar';},1500); }); }
    }
    ...
    function wireRes(){
        ...
        function recalcular(){
            ...
            var wEl=el('cs_wpp_txt'); if(wEl) wEl.innerHTML=esc(novoWpp).replace(/\n/g,'<br>');
        }
    ...
    function injectHTML(){
        ...
        o1.innerHTML='<div class="cs-hdr"><button class="btn-back" id="cs_close" style="flex-shrink:0"><i class="bi bi-arrow-left"></i></button><div class="cs-hdr-ico">🧠</div><div style="flex:1"><div class="cs-hdr-ttl">CreditoScan</div><div class="cs-hdr-sub">Análise de crédito via IA</div></div><button class="cs-btn-hist" id="cs_hist_open"><i class="bi bi-clock-history"></i> Histórico</button></div><div class="cs-body"><div class="cs-sec"><div class="cs-sec-ttl"><i class="bi bi-person-fill"></i> Cliente & Produto</div><div class="cs-fld"><label class="cs-lbl">Nome do cliente</label><input type="text" class="cs-in" id="cs_nome" placeholder="Ex: João da Silva" maxlength="80"></div><div class="cs-fld"><label class="cs-lbl">Produto a vender</label><div class="cs-row2"><input type="text" class="cs-in" id="cs_produto" placeholder="Ex: iPhone 13" maxlength="80"><div><label class="cs-lbl">Valor (R$)</label><input type="number" class="cs-in" id="cs_valor" placeholder="2200" min="1" step="1"></div></div></div></div><div class="cs-sec"><div class="cs-sec-ttl"><i class="bi bi-file-earmark-text-fill"></i> Documentos do Cliente<span class="cs-counter" id="cs_counter">0/'+MAX_DOCS+' arquivos</span></div><div class="cs-uz" id="cs_uz"><div class="cs-uz-ico">📄</div><div class="cs-uz-txt">Fotos ou PDF do extrato, contracheque ou holerite</div><div class="cs-uz-sub">Até '+MAX_DOCS+' arquivos · JPG PNG PDF</div></div><div class="cs-ubtns"><button class="cs-ubtn" id="cs_cam"><i class="bi bi-camera-fill"></i> Câmera</button><button class="cs-ubtn" id="cs_gal"><i class="bi bi-image-fill"></i> Galeria</button><button class="cs-ubtn" id="cs_pdf"><i class="bi bi-file-earmark-pdf-fill"></i> PDF</button></div><div class="cs-docs cs-h" id="cs_docs"></div></div><div id="cs_action_area" style="display:flex;flex-direction:column;gap:10px;"><button class="cs-btn-analisar" id="cs_btn_analisar" disabled><i class="bi bi-cpu-fill"></i> Analisar Crédito</button><div id="cs_btn_duo" class="cs-h" style="display:flex; gap:10px;"><button class="cs-btn-analisar" id="cs_btn_recalc" style="background:rgba(139,92,246,.15);border:1.5px dashed rgba(139,92,246,.4);color:#a78bfa;flex:1.2;font-size:.75rem;padding:10px;height:auto;"><div style="display:flex;flex-direction:column;align-items:center;"><span style="font-size:.85rem;margin-bottom:2px;font-weight:800"><i class="bi bi-arrow-repeat"></i> Atualizar Produto</span><span style="font-weight:normal;font-size:.65rem">0 Tokens</span></div></button><button class="cs-btn-analisar" id="cs_btn_nova" style="flex:1;font-size:.75rem;padding:10px;height:auto;"><div style="display:flex;flex-direction:column;align-items:center;"><span style="font-size:.85rem;margin-bottom:2px;font-weight:800"><i class="bi bi-stars"></i> Nova Análise</span><span style="font-weight:normal;font-size:.65rem">Gastar Token</span></div></button></div></div><div class="cs-prog cs-h" id="cs_prog"><div class="cs-prog-lbl" id="cs_prog_lbl">Processando...</div><div class="cs-prog-trk"><div class="cs-prog-bar" id="cs_prog_bar"></div></div></div><div class="cs-res cs-h" id="cs_res"></div></div>';
        document.body.appendChild(o1);

        var o2=document.createElement('div');o2.id='cs_hist_ov';
        o2.innerHTML='<div class="cs-hhdr"><button class="btn-back" id="cs_hist_close" style="flex-shrink:0"><i class="bi bi-arrow-left"></i></button><span class="cs-hhdr-ttl">Histórico de Consultas</span><span class="cs-hhdr-sub">últimos 30 dias</span></div><div class="cs-hbody" id="cs_hist_list"></div>';
    ...
    function injectMenuBtns(){
        ...
        if(goCS2&&!goCS2._csListenerOk){
            goCS2.addEventListener('click',openOverlay); goCS2._csListenerOk=true;
        } else if(!goCS2){
            var mg=el('menuCards');
            if(mg){
                var b2=document.createElement('button');b2.className='ctw-card c-rose';b2.id='goCS2';
                b2.innerHTML='<div class="ctw-card-icon"><i class="bi bi-cpu-fill"></i></div><span class="ctw-card-title">CreditoScan</span><span class="ctw-card-sub">Análise de crédito IA</span>';
